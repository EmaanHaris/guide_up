...
class CareerPathView extends StatefulWidget {
  final String menteeId;

  const CareerPathView({required this.menteeId, Key? key}) : super(key: key);

  @override
  _CareerPathViewState createState() => _CareerPathViewState();
}
class _CareerPathViewState extends State<CareerPathView>{
  ...
  void fetchCareer() async {
    ...
    try{
      ...
      final menteeId = widget.menteeId;
      await profileData.sendAndSaveCareerData(menteeId);
      final path=await profileData.fetchCareerPath(menteeId);
      final doc = await FirebaseFirestore.instance.collection('CareerPaths').doc(menteeId).get();
  ...
  Future<void> _loadCareerPath() async {
    ...
    final doc = await FirebaseFirestore.instance.collection('Mentees').doc(menteeId).get();
    if (doc.exists) {
      final data = doc.data();
...
class CareerStepWidget extends StatefulWidget {
  ...
  CareerStepWidget({required this.step, required this.stepIndex, required this.isNextStepCompleted,required this.isLastStep,  required this.fullCareerPath,  required this.onUpdate,Key? key,}) : super(key: key);
   @override
                                ...
                                children: [
                                  ...
                                  Column(
                                    children: widget.step.projects.asMap().entries.map((entry){
                                      int i = entry.key;
                                      Project project = entry.value as Project;
                                      return CheckboxListTile(
                                        ...
                                        onChanged: isLocked ?null:(val){
                                          setState(() {
                                            project.completed = val ?? false;
                                          });
                                          _saveCareerPath();
                                          widget.onUpdate(widget.step);
                                        }
                                      );
                                    }).toList(),
                                  ),
